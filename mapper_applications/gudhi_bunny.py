...
V, k = empiric_mod_of_contin(
    func=mpr.Projection(columns=[2]).fit(X).transform(X), 
    delta=delta,
    dist_mtrx= cdist(X,X)
    )
...
pipe_custom = mpr.make_mapper_pipeline(
    filter_func=mpr.Projection(columns=[2]),
    cover=cover,
...
if not os.path.exists("./mapper_applications/figures/"):
    os.mkdir("./mapper_applications/figures/")
filename = (
    "./mapper_applications/figures/bunny_Rips.svg"
)
fig.write_image(filename)
...
if not os.path.exists("./mapper_applications/figures/"):
    os.mkdir("./mapper_applications/figures/")
filename = (
    "./mapper_applications/figures/bunny_Automato.svg"
)
fig.write_image(filename)
